...
pub struct Ms {
    ...
    pub owner_key: Pubkey,            // random key(or not) used to seed the multisig pda
    pub allow_external_execute: bool, // allow non-member keys to execute txs
    pub guardian_keys: Vec<Pubkey>,   // keys of the members
    pub create_key: Pubkey,           // used to distinguish one wallet from the other
}
...
impl Ms {
    ...
    pub fn init(&mut self, owner_key: Pubkey, create_key: Pubkey, bump: u8) -> Result<()> {
        self.threshold = 0;
    ...
    pub fn is_guardian(&self, member: Pubkey) -> Option<usize> {
        match self.guardian_keys.binary_search(&member) {
            Ok(ind) => Some(ind),
            _ => None,
        }
    }
    ...
    pub fn add_guardian(&mut self, member: Pubkey) -> Result<()> {
        // TODO: Implement timelock and clockwork
        if matches!(self.is_guardian(member), None) {
            self.guardian_keys.push(member);
            self.guardian_keys.sort();
        }
        Ok(())
    }

    pub fn remove_guardian(&mut self, member: Pubkey) -> Result<()> {
        // TODO: Implement timelock and clockwork
        if let Some(ind) = self.is_guardian(member) {
            self.guardian_keys.remove(ind);
            if self.guardian_keys.len() < usize::from(self.threshold) {
                self.threshold = self.guardian_keys.len().try_into().unwrap();
            }
        }
        Ok(())
    }
    ...
    pub fn change_threshold(&mut self, threshold: u16) -> Result<()> {
        ...
        // TODO: Verify that the threshold is valid
        Ok(())
    }
}
...
impl IncomingInstruction {
    pub fn get_max_size(&self) -> usize {
        ...
        return get_instance_packed_len(&self)
            .unwrap_or_default()
            .checked_add(3)
            .unwrap_or_default();
    }
